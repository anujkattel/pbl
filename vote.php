...
$semester = $user['semester']; // Get user's semester

// Fetch general election candidates for user's semester
$stmt = $conn->prepare("SELECT * FROM votes WHERE status = 'approved' AND semester = :semester AND election_type != 'CAAS'");
$stmt->bindParam(':semester', $semester, PDO::PARAM_STR);
$stmt->execute();
$candidates = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch CAAS election candidates
$caas_stmt = $conn->prepare("SELECT * FROM votes WHERE status = 'approved' AND semester = :semester AND election_type = 'CAAS'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['vote']) && !$hasVoted) {
  ...
  // Validate the selected candidate with semester match
  $stmt = $conn->prepare("SELECT * FROM votes WHERE id = :candidate_id AND status = 'approved' AND semester = :semester");
